...
//   node scripts/record-video.mjs <url> <folder> [tilt 0-1] [turn 0-1] [speed]
// then, in the folder:
...
const [url, outdir, tilt = "0.5", turn = "0", speed = "2"] = process.argv.slice(2);
...
await c.goto(`${url}${sep}record&tilt=${+tilt * 100}&turn=${+turn * 360}&speed=${speed}`);
await sleep(500);
const frames = [];
c.on(async (m) => {
